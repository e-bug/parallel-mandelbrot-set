                                  ...
                                  double im2, int *rp, int *gp, int *bp)
{
  if(iter == maxiter) {
    ...
    *bp = 0; // Blue channel
    ...
    // Some constants used with colormap
    //double constant_inside = 1.75 - log2(0.5);
    //double denominator = log2((double)maxiter);
    int brightness = 256.*log2(1.75-log2(0.5)+iter-log2(log2(re2+im2)))/log2((double)maxiter);
